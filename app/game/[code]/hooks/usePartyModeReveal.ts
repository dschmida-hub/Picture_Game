...
export function computePartyModeRevealState(
  nowMs: number,
  revealStartedAt: string | null,
  submissionCount: number
) {
  if (!revealStartedAt || submissionCount === 0) {
    return { isSequenceActive: false, currentImageIndex: 0, secondsRemainingForImage: 0 };
  }

  const elapsedSeconds = Math.max(0, (nowMs - new Date(revealStartedAt).getTime()) / 1000);
...
export function usePartyModeReveal({
  ...
  return computePartyModeRevealState(now, revealStartedAt, submissionCount);
}
